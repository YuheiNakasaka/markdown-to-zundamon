...
```json
{
  "preprocess": "npx ts-node scripts/preprocess.ts",
  "studio": "npx remotion studio",
  "render": "npx remotion render ZundamonVideo out/video.mp4",
  "build": "npm run preprocess -- exmaple/example.md && npm run render"
}
```
...
2. `npm run preprocess -- exmaple/example.md` → `public/audio/` にWAV、`public/manifest.json` が生成されることを確認
3. `npm run studio` → Remotion Studioでプレビュー確認
4. `npm run render` → `out/video.mp4` が生成されることを確認
